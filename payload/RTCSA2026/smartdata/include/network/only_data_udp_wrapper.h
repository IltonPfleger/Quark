...
class Only_Data_UDP_Wrapper : public NIC_Common {
  protected:
    ...
    static const UInt32 HEADER_SIZE      = IPV4_HEADER_SIZE + UDP_HEADER_SIZE; // UDP Header Size
    static const UInt32 UDP_MTU          = 1522;
  ...
  public:
    ...
    class Header {
      public:
        Header() {}
        Header(const Address &src, const Address &dst, Protocol prot)
            : _dst(dst),
              _src(src),
              _prot(CPU::_htons(prot)) {}

        const Address &src() const { return _src; }
        const Address &dst() const { return _dst; }

        Protocol prot() const { return CPU::_ntohs(_prot); }

        friend OStream &operator<<(OStream &os, const Header &h) {
            os << "{d=" << h._dst << ",s=" << h._src << ",p=" << hex << h.prot() << dec << "}";
            return os;
        }
    ...
    // Data and Trailer
    static const UInt32 MTU = UDP_MTU;
    ...
    class Frame : public Header {
      public:
        ...
        Frame(const Address &src, const Address &dst, const Protocol &prot)
            : Header(src, dst, prot) {}
        Frame(const Address &src, const Address &dst, const Protocol &prot, const void *data, UInt32 size)
            : Header(src, dst, prot) {
...
#define _UTIL
    ...
    struct Configuration : public NIC_Common::Configuration {
        friend Debug &operator<<(Debug &db, const Configuration &c) {
            db << "{unit=" << c.unit << ",addr=" << c.address << ",a=" << c.timer_accuracy << ",f=" << c.timer_frequency
               << "}";
            return db;
        }
    ...
    struct Statistics : public NIC_Common::Statistics {
        ...
        friend Debug &operator<<(Debug &db, const Statistics &s) {
            db << "{rxp=" << s.rx_packets << ",rxb=" << s.rx_bytes << ",rxorun=" << s.rx_overruns
               << ",txp=" << s.tx_packets << ",txb=" << s.tx_bytes << ",txorun=" << s.tx_overruns
               << ",frm=" << s.frame_errors << ",car=" << s.carrier_errors << ",col=" << s.collisions << "}";
            return db;
        }
