...
template <typename Privilege> class TrapHandler {
  ...
  private:
    ...
    static size_t irq2index(ID id, Type type) { return id + (type * NumberOfExceptions); }
    ...
    template <bool ChangeStack> __attribute__((naked, optimize("O0"), aligned(4))) static void entry() {
