...
//         if ((context->status & MachineMode::PP) == MachineMode::PP_M)
...
//             uintptr_t address =
//             PageTable::virt2phys(csrr<MachineMode::TVAL>() + i); if
//             (!address) ExceptionHandler::esr(context); uint8_t data; if
//             (!VirtualCPU::lb(address, &data)) {
//                 ExceptionHandler::esr(context);
//             } else {
...
//             loaded       =
//             static_cast<uintmax_t>((static_cast<intmax_t>(loaded << shift))
//             >> shift);
//         }
//
//         if (rd != 0) {
