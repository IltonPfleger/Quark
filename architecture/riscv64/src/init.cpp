...
__attribute__((naked)) static void sjump() {
  static constexpr uint64_t MEDELEG =
  ...
  static constexpr uint64_t MIDELEG =
...
[[noreturn]] __attribute__((naked, optimize("O0"))) void epilogue() {
  ...
  CoreContextHandler<MachineMode>::bind(
      CoreContextHandler<MachineMode>::init(CPU::tp()));
...
extern "C" __attribute__((optimize("O0"), naked, used, section(".init"))) void
_init() {
  size_t core;
  uintptr_t position;
  ...
  // Get The Boot Stack
  asm("mv sp, %0" ::"r"(position + Traits<Memory>::Size -
