...
public:
  ...
  [[nodiscard]] static constexpr bool floating(uint32_t instruction) {
    ...
    return (opcode == LD) || (opcode == SD) || (opcode == OP) ||
           (opcode == FMADD) || (opcode == FMSUB) || (opcode == FNMSUB) ||
           (opcode == FNMADD);
  }
  ...
  static uint8_t rs2(uint16_t instruction16) {
    return 8 + ((instruction16 >> 2) & 0x7);
  }
  static uint8_t rs2(uint32_t instruction) {
    return (instruction >> 20) & 0x1f;
  }

  static uint8_t funct3(uint32_t instruction) {
    return (instruction >> 12) & 0x7;
  }
  static uint8_t funct3(uint16_t instruction16) {
    return (instruction16 >> 13) & 0x7;
  }

  static uint8_t rs1(uint32_t instruction) {
    return (instruction >> 15) & 0x1F;
  }
  static uint8_t rs1(uint16_t instruction16) {
    return 8 + ((instruction16 >> 7) & 0x7);
  }

  static uint16_t csr(uint32_t instruction) {
