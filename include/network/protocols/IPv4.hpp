...
class IPv4 : public Observer<const NetworkBuffer *>,
             public Observed<const NetworkBuffer *, const NetworkAddress &,
                             const NetworkAddress &, uint8_t> {
public:
  ...
  struct Header {
    ...
    Header(Address d, Address s, uint8_t p, uint16_t l, uint8_t t = 0,
           uint16_t id = 0, uint16_t n = 0, uint8_t time = DefaultTTL) {
      version = VersionIHL;
      tos = t;
  ...
  IPv4(const Address &address, NetworkLinkLayer &link) : link_(link) {
    _address = address;
    link_.address(_address);
    link_.attach(this);
  }
