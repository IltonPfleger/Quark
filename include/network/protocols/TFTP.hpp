...
public:
  TFTP(UDP &udp) : udp_(udp), handler_(0), state_(State::DONE) {
    udp_.attach(this);
  }

  ~TFTP() { udp_.detach(this); }

  int request(const NetworkAddress &&address, const char *filename,
    ...
    if (state_ != State::DONE || received_ == 0)
      return -1;

    return received_;
  }

  void update(const NetworkBuffer *received, uint16_t,
              uint16_t source) override {
