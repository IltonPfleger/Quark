...
public:
  JH7110_DVFS_Controller() : i2c_(), pmic_(i2c_) {}
  ...
  virtual Span<const PState> available() override {
    return Span(States, sizeof(States) / sizeof(PState));
  }
  ...
  uintmax_t frequency() {
    return PLL0::rate() /
           Clock_Controller::divisor(Clock_Controller::SYSCRG_CLK_CPU_CORE);
  }
