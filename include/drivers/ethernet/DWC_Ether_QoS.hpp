...
public:
  ...
  static void write(unsigned char phy, unsigned char dev, unsigned short data) {
    ...
    Reg32(Base, BASE) =
        BUSY | WRITE | ((phy & 0x1F) << 21) | ((dev & 0x1F) << 16);
    wait();
  }
  static unsigned short read(unsigned char phy, unsigned char dev) {
    Reg32(Base, BASE) =
        BUSY | READ | ((phy & 0x1F) << 21) | ((dev & 0x1F) << 16);
    wait();
    return Reg32(Base, DATA) & 0xFFFF;
  }

  static void write45(unsigned char phy, unsigned short reg,
                      unsigned short data) {
    write(phy, 0x1E, reg);
    write(phy, 0x1F, data);
  }

  static void set45(unsigned char phy, unsigned short reg,
                    unsigned short data) {
    write45(phy, reg, read45(phy, reg) | data);
  }

  static void clear45(unsigned char phy, unsigned short reg,
                      unsigned short data) {
    write45(phy, reg, read45(phy, reg) & ~data);
  }
...
public:
  ...
  static bool negotiate() {
    ...
    while (!(MDIO::read(Phy, BASIC_STATUS) &
      ...
      ;
    while (!((MDIO::read(Phy, MASTER_SLAVE_STATUS_REGISTER) &
...
template <typename MyTraits> class DWC_Ether_QoS_DMA : public Driver {
  typedef Meta::GetFromTypeList<Traits<QUARK::CacheController>::Devices,
                                0>::Result Cache;
  ...
  struct Descriptor {
    ...
    template <typename T = uintptr_t> T buffer() const {
      uint64_t pointer =
          (static_cast<uint64_t>(des1) << 32) | static_cast<uint64_t>(des0);
...
public:
  static void init() {
    TraceIn();
    Reg32(Base, TX_QUEUE0_OPERATION_MODE) =
        TX_QUEUE0_OPERATION_MODE_TSF | TX_QUEUE0_OPERATION_MODE_ENABLE;
    Reg32(Base, RX_QUEUE0_OPERATION_MODE) =
        RX_QUEUE0_OPERATION_MODE_FUP | RX_QUEUE0_OPERATION_MODE_RSF;
    TraceOut();
  }
};
