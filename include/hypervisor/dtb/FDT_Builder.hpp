...
public:
  FDT_Builder(void *buffer, size_t capacity)
      : buffer_(reinterpret_cast<uint8_t *>(buffer)), capacity_(capacity),
        cursor_(sizeof(FDT_Header)), strings_cursor_(capacity) {
    write64(0);
    write64(0);
  }
  ...
  void add(const char *name, const char *value) {
    add_property(name, value, strlen(value) + 1);
  }
  void add(const char *name, uint32_t value) {
    add_property(name, &((value = CPU::htobe32(value))), sizeof(value));
  }
  ...
  size_t finish() {
    ...
    while (p + 4 < end) {
      ...
      if (tag == FDT_BEGIN_NODE) {
        ...
        assert(p + 12 <= end);
        uint32_t length =
            CPU::be32toh(*reinterpret_cast<uint32_t *>(buffer_ + p + 4));
        uint32_t string =
            CPU::be32toh(*reinterpret_cast<uint32_t *>(buffer_ + p + 8));
    ...
    header->off_dt_struct =
        CPU::htobe32(sizeof(FDT_Header) + sizeof(FDT_ReservedEntry));
    ...
    header->size_dt_struct =
        CPU::htobe32(end - sizeof(FDT_Header) + sizeof(FDT_ReservedEntry));
