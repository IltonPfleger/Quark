...
{
protected:
    static const UInt32 HEADER_SIZE = 14;
...
public:
    ...
    public:
        Header() {}
        Header(const Address & src, const Address & dst, const Protocol & prot): _dst(dst), _src(src), _prot(CPU::_htons(prot)) {}
    ...
    public:
        ...
        Frame(const Address & src, const Address & dst, const Protocol & prot, const void * data, UInt32 size): Header(src, dst, prot) { memcpy(_data, data, size); }
