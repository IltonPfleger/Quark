...
class ETSI_CPM : public Transducer<SmartData::Unit::MOTION_VECTOR_GLOBAL| 30>, private UDP_Socket::Observer
{
    friend Responsive_SmartData<ETSI_CPM>;
...
public:
    ETSI_CPM(const Device_Id & dev): _value(), _signature(0) {
        _my_socket = new UDP_Socket(dev+1);
        _my_socket->attach(this);
    }

    ~ETSI_CPM() { delete _my_socket; }
    ...
    void update(typename UDP_Socket::Observed *obs) {
        ...
        db<SmartData>(TRC) << "New CPM received! t=" << _my_socket->reception() << endl;
    }
...
using ETSI_CPM_Source = Responsive_SmartData<ETSI_CPM>;