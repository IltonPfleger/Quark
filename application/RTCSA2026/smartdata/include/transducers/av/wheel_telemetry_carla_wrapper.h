...
class Wheel_Telemetry_Reader: public Transducer<SmartData::Unit::WHEEL_TELEMETRY>, private UDP_Socket::Observer
{
    friend Responsive_SmartData<Wheel_Telemetry_Reader>;
...
public:
    Wheel_Telemetry_Reader(const Device_Id & dev): _value(0) {
        // 5003 --> (2+1) Image socket (Carla -> SmartData)
        _my_socket = new UDP_Socket(dev+1);
        _my_socket->attach(this);
    }

    ~Wheel_Telemetry_Reader() { delete _my_socket; }
...
using Wheel_Telemetry = Responsive_SmartData<Wheel_Telemetry_Reader>;
