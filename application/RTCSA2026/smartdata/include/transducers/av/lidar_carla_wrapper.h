...
class LIDAR_Transducer: public Transducer<SmartData::Unit::PCD_MONOCROMATIC|1>, private UDP_Socket::Observer
{
    friend Responsive_SmartData<LIDAR_Transducer>;
...
public:
    LIDAR_Transducer(const SmartData::Device_Id & dev): _value() {
        _my_socket = new UDP_Socket(dev+1);
        _my_socket->attach(this);
        _uncertainty = UNKNOWN;
    }

    ~LIDAR_Transducer() { delete _my_socket; }
    ...
    void update(typename UDP_Socket::Observed *obs) {
        db<SmartData>(TRC) << "LIDAR Update!" << endl;
        ...
        if (received < UDP_Socket::MTU) {
            ...
            for (UInt32 i = _index; i < sizeof(Value); i++) // erase remaning bytes
                _value[i] = 0;
            _index = 0;
            db<SmartData>(INF) << "Last fragment of LIDAR received! size=" << received << " t=" << _my_socket->reception() << endl;
            notify();
            ...
            memcpy(&_value[_index], _my_socket->data(), received);
            _index += received;
            db<SmartData>(INF) << "New fragment of LIDAR received! size=" << received << "t=" << _my_socket->reception() << endl;
        }
    }
...
using LIDAR_AV = Responsive_SmartData<LIDAR_Transducer>;
