...
class Fuel_Consumption_Transducer: public Transducer<SmartData::Unit::Mass_Flow |SmartData::Unit::D64>, private UDP_Socket::Observer
{
    friend Responsive_SmartData<Fuel_Consumption_Transducer>;
public:
...
public:
    Fuel_Consumption_Transducer(const Device_Id & dev): _value(0), _signature(0) {
        _my_socket = new UDP_Socket(dev+1);
        _my_socket->attach(this);
    }

    ~Fuel_Consumption_Transducer() { delete _my_socket; }
...
using Fuel_Consumption = Responsive_SmartData<Fuel_Consumption_Transducer>;
