...
// Observed x Observer is used in mediators, so they payloadear very early in the system.
...
public:
    ...
    virtual UInt32 observers() const { return _observers.size(); }
...
public:
    ...
    virtual _Observer * observer(const C & c, UInt32 index = 0) {
        _Observer * o = 0;
        for(Element * e = _observers.head(); e; e = e->next()) {
            if(e->rank() == c) {
                if(!index)
...
public:
    typedef typename Simple_List<Data_Observer<D, void>>::Iterator Iterator;
...
public:
    ...
    virtual _Observer * observer(UInt32 index = 0) {
        _Observer * o = 0;
        for(Element * e = _observers.head(); e; e = e->next()) {
            if(!index)
                o =  e->object();
