...
namespace List_Elements
{
    ...
    public:
        Singly_Linked_Grouping(const T * o, UInt32 s): _object(o), _size(s), _next(0) {}
        ...
        void size(UInt32 l) { _size = l; }
        void shrink(UInt32 n) { _size -= n; }
        void expand(UInt32 n) { _size += n; }
    ...
    public:
        Doubly_Linked_Grouping(const T * o, UInt32 s): _object(o), _size(s), _prev(0), _next(0) {}
...
public:
    ...
    UInt32 grouped_size() const { return _grouped_size; }

    Element * search_size(UInt32 s) {
        Element * e = head();
    ...
    Element * search_decrementing(UInt32 s) {
...
{
    template<typename FT, typename FR, typename FEl, UInt32 FH>
    friend class Multihead_Scheduling_List;     // for chosen() and remove()
    template<typename FT, typename FR, typename FEl, typename FL, UInt32 FQ>
...
// the head to which the current operation payloadlies.
template<typename T,
          typename R = typename T::Criterion,
          typename El = List_Elements::Doubly_Linked_Scheduling<T, R>,
          UInt32 H = R::HEADS>
...
// queue to which the current operation payloadlies, and the queue() method to
...
public:
    ...
    UInt32 size() const { return _list[R::current_queue()].size(); }
    ...
    Iterator begin(UInt32 queue) { return Iterator(_list[queue].head()); }
    Iterator end() { return Iterator(0); }
...
// queue to which the current operation payloadlies, the current_head() class
// method to designate the head to which the current operation payloadlies, and
