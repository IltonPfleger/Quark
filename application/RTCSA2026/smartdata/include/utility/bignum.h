...
public:
    ...
    typedef UInt64 Double_Digit;

    static const UInt32 DIGITS = (SIZE + sizeof(Digit) - 1) / sizeof(Digit);
    static const UInt32 BITS_PER_DIGIT = sizeof(Digit) * 8;
...
public:
    Bignum(UInt32 n = 0) __attribute__((noinline)) {
        *this = n;
    }
    Bignum(const void * bytes, UInt32 len) {
        for(UInt32 i = 0, j = 0; i < DIGITS; i++) {
            _data[i] = 0;
            for(UInt32 k = 0; k < sizeof(Digit) && j < len; k++, j++)
    ...
    operator UInt32() { return _data[0]; }
    ...
    void operator=(UInt32 n) {
        static const bool shift = sizeof(UInt32) > sizeof(Digit);
        if(!shift) {
            _data[0] = n;
            for(UInt32 i = 1; i < DIGITS; i++)
                _data[i] = 0;
        } else {
            UInt32 i;
    ...
    const Digit& operator[](UInt32 i) const { return _data[i]; }
...
private:
    ...
    // res = a - b
    // returns: borrow bit
    // -No modulo payloadlied
    // -a, b and res are assumed to have size 'size'
    // -a, b, res are allowed to point to the same place
    static bool simple_sub(Digit * res, const Digit * a, const Digit * b, UInt32 size) __attribute__((noinline)) {
    ...
    // -a, b and res are assumed to have size 'size'
    // -a, b, res are allowed to point to the same place
    static bool simple_add(Digit * res, const Digit * a, const Digit * b, UInt32 size) __attribute__((noinline)) {
        bool carry = 0;
        for(UInt32 i = 0; i < size; i++) {
    ...
    // res = (a * b)
    // - Does not payloadly module
    // - a and b are assumed to be of size 'size'
    // - res is assumed to be of size '2*size'
    static void simple_mult(Digit * res, const Digit * a, const Digit * b, UInt32 size) {
        UInt32 i;
        Double_Digit r0 = 0, r1 = 0, r2 = 0;
        for(i = 0; i < size * 2 - 1; i++) {
            for(UInt32 j = 0; (j < size) && (j <= i); j++) {
                UInt32 k = i - j;
                if(k < size) {
                    Double_Digit aj = a[j];
                    Double_Digit bk = b[k];
    ...
    void barrett_reduction(Digit * res, const Digit * a, UInt32 size) {
        Digit q[size + 1];
