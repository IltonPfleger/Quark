...
{
private:
    static const UInt32 SIZE = N * sizeof(T);
...
public:
    Array() {}
    Array(const void * data, UInt32 size) { copy_and_pad(data, size); }
    template <typename U>
    Array(const U & o) { copy_and_pad(&o, sizeof(U)); }

    static constexpr UInt32 length() { return N; };

    Object_Type & operator[](const UInt32 i) { return _data[i]; }
    const Object_Type & operator[](const UInt32 i) const { return _data[i]; }
    ...
    UInt32 search(const Object_Type & obj) {
        UInt32 i = 0;
        for(; i < N; i++)
            if(_data[i] == obj)
                break;
...
class Array<T, 0> {
public:
    static constexpr UInt32 length() { return 0; };
};
