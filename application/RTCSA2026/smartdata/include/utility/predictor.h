...
public:
    ...
    template<UInt32 T, typename _Model>
    class Model: public _Model
    {
    public:
        static const UInt32 TYPE = T;
    ...
    template<History_Type TYPE, UInt32 SIZE, typename ... Tn>
...
private:
    ...
    static const UInt32 MAX_WINDOW = 100;
...
public:
    ...
    struct Configuration
    {
        Configuration(Value r = 0, Value a = 0, Time t = 0, UInt32 w = 0, UInt32 p = 0)
...
public:
    DBP(UInt32 w, UInt32 p, Value r = 0, Value a = 0, Time t = 0): _ready(false), _config(w, p, r, a, t), _miss_predicted(0) {
...
protected:
    void build_model(const Time & t, const Value & v){
        ...
        for(UInt32 i = 0; i < _config.points; i++)
            avg_oldest += _history[i].value();
        avg_oldest /= _config.points;

        for(UInt32 i = 0; i < _config.point; i++)
