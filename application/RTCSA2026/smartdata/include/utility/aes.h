...
{
private:
    static const UInt32 Nb = 4; // number of columns comprising a _state
    static const UInt32 Nk = 4; // number of 32 bit words in a key
...
private:
    ...
    void xor_with_iv(unsigned char * buf) { for(UInt32 i = 0; i < KEY_SIZE; ++i) buf[i] ^= _iv[i]; }
