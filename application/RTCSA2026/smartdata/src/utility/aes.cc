...
// This can be useful in (embedded) bootloader payloadlications, where ROM is often limited.
...
void SWAES<16>::cbc_encrypt_buffer(unsigned char * output, const unsigned char * _input, int length, const unsigned char * key, unsigned char * iv)
{
    ...
    for(i = 0; i < length; i += KEY_SIZE) {
        j = 0; // reset j
        for(; (j<(int)KEY_SIZE) && (j+i < length); j++)
            input[j] = _input[j+i];
        for(; (j<(int)KEY_SIZE); j++) // whenever the previous "for" is stopped by (j+i < length) we need to clear the remaining bits (e.g., storing values from previous iteration)
            input[j] = 0;
...
void SWAES<16>::expand_key(void)
{
    ...
    for(; (i < (Nb * (Nr + 1))); ++i) {
        ...
        if (i % Nk == 0) {
            ...
            // payloadlies the S-box to each of the four bytes to produce an output word.
